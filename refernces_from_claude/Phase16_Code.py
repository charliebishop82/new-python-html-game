...
<table>
    ...
    <tr>
        ...
        <td>{{ p.username }}</td>
        <td>{{ p.character_name }} {% if p.is_npc %}<span style="color:#4499ff">[NPC]</span>{% endif %}</td>
        <td>{{ p.level }}</td>
        <td>{{ p.current_hp }}</td>
        <td>{{ p.current_ap }}</td>
        <td>{{ p.credits }}</td>
...
<p style="color:#666;margin-bottom:16px;">NPC wake times are staggered through the UTC day. AP alone limits their actions, with catch-up attempts during the final hour.</p>
...
<form id="create-npc-form" method="POST" action="/admin/npcs" style="display:grid;grid-template-columns:repeat(4,minmax(150px,1fr));gap:10px;background:#111;padding:12px;border:1px solid #222;">
    ...
    <label>Preset<br><select id="npc-preset" name="preset"><option value="hunter">Player Hunter</option><option value="boss">Boss Killer</option><option value="hoarder">Hoarder</option><option value="thief">Thief</option><option value="hybrid">Hybrid</option><option value="custom">Custom</option></select></label>
...
<section style="margin-top:20px;padding:12px;border:1px solid #333;background:#0d0d0d;opacity:{{ '0.55' if npc.retired else '1' }}">
    ...
    <form method="POST" action="/admin/npcs/{{ npc.player_id }}/edit" style="display:flex;flex-wrap:wrap;gap:8px;align-items:end">
        {% for field,label in [('player_hunter','Hunter'),('boss_killer','Boss'),('hoarder','Hoarder'),('thief','Thief'),('aggression','Aggression'),('self_preservation','Safety'),('repair_tendency','Repair')] %}
    ...
    {% endif %}
    <table><tr><th>Inventory Item</th><th>Type</th><th>Durability</th><th></th></tr>
    {% for item in inventory if item.player_id == npc.player_id %}<tr><td>{{ item.item_name }}</td><td>{{ item.item_type }}</td><td>{{ item.current_durability }}%</td><td>{% if not npc.retired %}<form method="POST" action="/admin/npcs/{{ npc.player_id }}/inventory/{{ item.id }}/remove"><button class="btn btn-danger">Remove</button></form>{% endif %}</td></tr>{% else %}<tr><td colspan="4" style="color:#666">No inventory</td></tr>{% endfor %}</table>
</section>
{% else %}<p style="color:#666;margin-top:20px">No NPCs created yet.</p>{% endfor %}

<h2>Recent NPC Decisions</h2>
<table><tr><th>Time</th><th>NPC</th><th>Decision</th><th>Reason</th><th>Result</th></tr>{% for log in logs %}<tr><td>{{ log.occurred_at[:16] }}</td><td>{{ log.character_name }}</td><td>{{ log.decision }}</td><td>{{ log.reason }}</td><td>{{ log.result }}</td></tr>{% endfor %}</table>
<script>
(() => {
  ...
  const presets = {
    hunter:  {player_hunter:100,boss_killer:0,hoarder:0,thief:0,aggression:85,self_preservation:35,repair_tendency:55,description:'Prioritizes eligible PvP opponents and fights aggressively.'},
    boss:    {player_hunter:0,boss_killer:100,hoarder:0,thief:0,aggression:65,self_preservation:55,repair_tendency:75,description:'Prioritizes boss progression, observation, and maintaining reliable gear.'},
    hoarder: {player_hunter:0,boss_killer:0,hoarder:100,thief:0,aggression:25,self_preservation:85,repair_tendency:80,description:'Prioritizes collecting specials and protecting its inventory.'},
    thief:   {player_hunter:0,boss_killer:0,hoarder:0,thief:100,aggression:55,self_preservation:70,repair_tendency:65,description:'Alternates random legal PvP theft attempts with boss fights for progression.'},
    hybrid:  {player_hunter:60,boss_killer:60,hoarder:60,thief:0,aggression:55,self_preservation:55,repair_tendency:65,description:'Balances PvP, bosses, and collecting with no single dominant motivation.'}
  };
