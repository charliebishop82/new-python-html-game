################################################################################
# PHASE 5 CODE — Boss/Minion Combat Routes + Combat Fragments
# BBS-Inspired Multiplayer Dueling Game
#
# Files included:
#   1. routes/actions.py   — Full replacement of Phase 3 stub:
#                            random event check, boss/minion fight initiation,
#                            PvP opponent list + fight initiation, tavern (carried forward)
#   2. routes/combat.py    — Full replacement of Phase 3 stub:
#                            combat round handler, steal confirmation, PvP extension,
#                            score-formula resolution
#   3. templates/fragments/ — All combat HTML fragments:
#                            event_result, boss_confirm, minion_spotted,
#                            opponent_list, combat_open, combat_round,
#                            combat_steal_confirm, combat_extend, combat_result
#
# IMPORTANT: These files REPLACE (not supplement) the Phase 3 stubs for
# routes/actions.py and routes/combat.py.
#
# Queue handlers registered in this phase:
#   start_boss_fight, start_pvp_fight,
#   combat_action, combat_steal, combat_extend, combat_resolve
#
# Requires Phase 1-4 files to already be in place.
################################################################################

################################################################################
# FILE: routes/actions.py  (Phase 5 — replaces Phase 3 stub, full implementation)
################################################################################
...
def _describe_random_event_effect(event: dict) -> str:
    ...
    stat_names = {
        ...
        "STAT_BOOST_LCK": "LCK", "STAT_BOOST_PER": "PER", "STAT_BOOST_INITIATIVE": "Initiative",
        "STAT_PENALTY_STR": "STR", "STAT_PENALTY_END": "END", "STAT_PENALTY_AGI": "AGI",
        "STAT_PENALTY_LCK": "LCK", "STAT_PENALTY_PER": "PER", "STAT_PENALTY_INITIATIVE": "Initiative",
    }
    if effect in stat_names: return f"{stat_names[effect]} {amount:+d}"
...
################################################################################
# FILE: routes/combat.py   (Phase 5 — replaces Phase 3 stub, full implementation)
################################################################################
...
################################################################################
# FILE: templates/fragments/ (all combat fragments)
################################################################################

<!-- ============================================================ -->
<!-- FILE: templates/fragments/event_result.html               -->
<!-- ============================================================ -->
<div class="fragment"
...
<!-- ============================================================ -->
<!-- FILE: templates/fragments/boss_confirm.html               -->
<!-- ============================================================ -->
<div class="fragment">
    <div class="term-line term-amber">
        ⚠ WARNING — LEVEL MISMATCH
    </div>
    <div class="term-line term-white">
        {{ opponent.name }} is {{ level_diff }} level(s) above you.
        This fight may be extremely dangerous.
    </div>
    <div class="term-line term-grey" style="font-size:11px;">
        {{ opponent.flavor_text }}
    </div>
    <div style="display:flex;gap:8px;margin-top:8px;">
        <form class="terminal-action"
              action="{{ url_for('actions.action_boss_confirm') }}" method="POST">
            ...
            <button type="submit" class="action-btn" style="width:auto;padding:6px 16px;">
                Fight Anyway
            </button>
        </form>
        <form class="terminal-action"
            ...
            <input type="hidden" name="opponent_id"    value="{{ opponent.id }}">
            <input type="hidden" name="encounter_type" value="{{ encounter_type }}">
            <input type="hidden" name="action"         value="avoid">
            <button type="submit" class="action-btn" style="width:auto;padding:6px 16px;">
                Back Away (No AP cost)
            </button>
        </form>
    </div>
</div>
...
<!-- ============================================================ -->
<!-- FILE: templates/fragments/minion_spotted.html             -->
<!-- ============================================================ -->
<div class="fragment">
    <div class="term-line term-amber">
        ★ MINION SPOTTED — {{ minion.name }}
    </div>
    ...
    <div style="display:flex;gap:8px;margin-top:8px;">
        <form class="terminal-action"
              action="{{ url_for('actions.action_boss_confirm') }}" method="POST">
            <input type="hidden" name="opponent_id"    value="{{ minion.id }}">
            <input type="hidden" name="encounter_type" value="MINION">
            <input type="hidden" name="action"         value="fight">
            <button type="submit" class="action-btn" style="width:auto;padding:6px 16px;">
                Engage
            </button>
...
<!-- ============================================================ -->
<!-- FILE: templates/fragments/opponent_list.html              -->
<!-- ============================================================ -->
<div class="fragment">
    <div class="term-line term-amber">
        ═══ PVP — CHOOSE YOUR OPPONENT ═══════════════════════════════
    </div>
    {% if opponents %}
    <table style="margin-top:8px;">
        <tr>
            <th>Name</th><th>Level</th><th>HP</th><th>Wealth</th><th>Status</th><th></th>
        </tr>
        {% for opp in opponents %}
        <tr>
            <td>{{ opp.character_name }}</td>
            <td>{{ opp.level }}</td>
            <td class="term-{{ 'good' if opp.hp_tier == 'Healthy'
                                else 'bad' if opp.hp_tier == 'Critical'
                                else 'amber' }}">
                {{ opp.hp_tier }}
            </td>
            <td style="color:var(--grey)">{{ opp.wealth_tier }}</td>
            <td>
                {% if opp.in_combat %}
                <span style="color:var(--red)">In Combat</span>
                {% else %}
                <span style="color:var(--grey)">Available</span>
                {% endif %}
            </td>
            <td>
                {% if not opp.in_combat %}
                <form class="terminal-action"
                      action="{{ url_for('actions.action_pvp_fight') }}" method="POST">
                    <input type="hidden" name="target_id" value="{{ opp.id }}">
                    <button type="submit" class="btn-small">Fight</button>
                </form>
                {% endif %}
            </td>
        </tr>
    ...
    {% else %}
    <div class="term-line term-grey">
        No eligible opponents right now. Everyone is either too low level,
        at 1 HP, or in combat.
    </div>
    {% endif %}
</div>
...
<!-- ============================================================ -->
<!-- FILE: templates/fragments/combat_open.html                -->
<!-- ============================================================ -->
<div class="fragment"
    ...
    {% if intel %}
    <div class="term-line term-blue">
        [KNOWN INTEL from previous encounter]
    </div>
    {% endif %}
    ...
    <div id="combat-actions" style="margin-top:10px;display:flex;flex-wrap:wrap;gap:6px;">
        <form class="terminal-action"
              action="{{ url_for('combat.action') }}" method="POST">
            <input type="hidden" name="action_type" value="attack">
            <button type="submit" class="action-btn" style="width:auto;padding:6px 14px;">
                ⚔ Attack
            </button>
        </form>
        <form class="terminal-action"
              action="{{ url_for('combat.steal') }}" method="POST">
            <button type="submit" class="action-btn" style="width:auto;padding:6px 14px;">
                💰 Steal
            </button>
        </form>
...
<script>
// Store combat session id so JS can use it
window._combatSessionId = {{ session_id }};
</script>


<!-- ============================================================ -->
<!-- FILE: templates/fragments/combat_round.html               -->
<!-- ============================================================ -->
<div class="fragment"
     data-hp="{{ player.current_hp }}"
     data-max-hp="{{ player.max_hp }}"
    ...
    <div class="term-line term-grey" style="font-size:11px;margin-left:12px;">
        {{ action_result.roll_detail if action_result.get('roll_detail') else '' }}
    </div>
    {% endfor %}
...
<!-- ============================================================ -->
<!-- FILE: templates/fragments/combat_steal_confirm.html       -->
<!-- ============================================================ -->
<div class="fragment">
    <div class="term-line term-amber">
        💰 STEAL ATTEMPT — Confirm?
    </div>
    <div class="term-line term-grey">
        Opposing {{ opponent.character_name or opponent.name }}.
        Success: take an item or credits. Failure: your AC is reduced for the next hit.
    </div>
    <div style="display:flex;gap:8px;margin-top:8px;">
        <form class="terminal-action"
              action="{{ url_for('combat.steal_confirm') }}" method="POST">
            <button type="submit" class="action-btn" style="width:auto;padding:6px 14px;">
                Confirm Steal
            </button>
        </form>
...
<!-- ============================================================ -->
<!-- FILE: templates/fragments/combat_extend.html              -->
<!-- ============================================================ -->
<div class="fragment" id="extend-fragment">
    <div class="term-line term-amber">
        ═══ ROUND LIMIT REACHED ══════════════════════════════════════
    </div>
    <div class="term-line term-white">
        The fight is unresolved. Extend or resolve by score formula?
    </div>
    <div class="term-line term-system">
        Auto-resolving in <span id="extend-timer">{{ timeout }}</span> seconds...
    </div>
    <div style="display:flex;gap:8px;margin-top:8px;">
        <form class="terminal-action"
              action="{{ url_for('combat.extend') }}" method="POST">
            <button type="submit" class="action-btn" style="width:auto;padding:6px 14px;">
                Extend (1 AP)
            </button>
        </form>
        <form class="terminal-action"
              action="{{ url_for('combat.resolve') }}" method="POST"
              id="resolve-form">
            <button type="submit" class="action-btn" style="width:auto;padding:6px 14px;">
                Resolve Now
            </button>
...
startExtensionTimer(
    {{ timeout }},
    "{{ url_for('combat.resolve') }}"
);
</script>


<!-- ============================================================ -->
<!-- FILE: templates/fragments/combat_result.html              -->
<!-- ============================================================ -->
<div class="fragment"
     data-hp="{{ player.current_hp }}"
     data-max-hp="{{ player.max_hp }}"
    ...
    <div class="term-line term-{{ 'good' if won else 'bad' }}">
        ═══ COMBAT OVER — {{ 'VICTORY!' if won else 'DEFEAT' }} ══════════════════
    </div>
