...
Failed choices hand off to a separate three-actor scene-combat engine. That
engine has its own sessions and logs and does not alter ordinary combat.
"""
...
def scene_with_choices(scene_id: int) -> dict | None:
    ...
    scene["choices"] = execute(
        "SELECT * FROM scene_choices WHERE scene_id=? AND is_active=1 ORDER BY CASE attribute "
...
def start_scene(player_id: int, scene_id: int | None = None) -> dict:
    ...
    if player["in_combat"] or player.get("in_scene_combat"):
...
def resolve_choice(player_id: int, attempt_id: int, choice_id: int,
    ...
    choice = execute_one(
        "SELECT * FROM scene_choices WHERE id=? AND scene_id=? AND is_active=1",
    ...
    resolved_at = None if needs_combat else now
    with exclusive_transaction():
        execute_write(
            ...
            (choice_id, status, roll, bonus, total, choice["difficulty"], int(succeeded),
             xp, credits, int(first), outcome_text, resolved_at, attempt_id),
        )
        if xp or credits:
    ...
    return {"attempt_id": attempt_id, "scene_name": attempt["scene_name"],
            ...
            "credits": credits, "effect": "", "effect_value": 0,
